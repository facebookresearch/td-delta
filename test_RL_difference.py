...
"""
Modified codebase from the paper: How Many Random Seeds? Statistical Power Analysis in Deep Reinforcement Learning Experiments, if use please cite original authors.
...
games = ["AlienNoFrameskip-v4", "AmidarNoFrameskip-v4", "BankHeistNoFrameskip-v4",  "FrostbiteNoFrameskip-v4", "HeroNoFrameskip-v4", "MsPacmanNoFrameskip-v4", "QbertNoFrameskip-v4", "WizardOfWorNoFrameskip-v4", "ZaxxonNoFrameskip-v4"]
for game in games:
    index = games.index(game)
    print(game)

    b1 = pd.read_csv('ppo_per_run_random_results.csv').loc[[index]].values[0][1:]
    print("PPO {} $\\pm$ {}".format(int(np.mean(b1)), int(stats.sem(b1))))
    b2 = pd.read_csv('ppo+_per_run_random_results.csv').loc[[index]].values[0][1:]
    print("PPO+ {} $\\pm$ {}".format(int(np.mean(b2)), int(stats.sem(b2))))

    data1 = b1 if np.mean(b1) > np.mean(b2) else b2
    data2 = pd.read_csv('ppoDeltaCappedLambda_per_run_random_results.csv').loc[[index]].values[0][1:]
    print("PPO-Delta capped{} $\\pm$ {}".format(int(np.mean(data2)), int(stats.sem(data2))))
